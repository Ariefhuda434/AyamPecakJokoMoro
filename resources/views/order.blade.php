...
@section('content')
    ...
    <div class="min-h-screen w-full px-4 pt-4 pb-10">
        ...
        <a hre="{{ route('order.index') }}" class="flex  gap-4 mb-8">
        ...
        <a hre="{{ route('order.index',['status_table' => 'Kosong']) }}" class="flex  gap-4 mb-8">
            <div class="flex p-4 w-53 h-23 bg-secondary text-primary rounded-2xl shadow-md hover:text-background transition-all duration-400 hover:scale-[1.02] hover:shadow-lg ease-in-out hover:bg-primary shadow-md">
                <div class="h-15 w-15 bg-white mr-3 rounded-md"></div>
                <div class="flex flex-col">
        ...
        <div class="w-full p-4 bg-white border-4 border-gray-300 rounded-2xl shadow-lg">
            ...
            <form action="{{ route('make.customer', $table->No_Table) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="No_Table" value="{{ $table->No_Table}}"
                        class="hidden" />
                </div>
                ...
                <div class="mb-4">
                    <input type="text" name="Notes" placeholder="Catatan" value="{{ old('Notes') }}"
                        class="w-full h-11 border-4 border-gray-300 rounded-xl rounded-bl-3xl 
                            px-4 outline-none transition-all focus:border-secondary duration-300 ease-in-out
                            placeholder-gray-500 font-sans text-base" />
                </div>
            ...
            @else
            @php
        $customer = $table->activeCustomer; 
        @endphp
             <div class="w-full p-4  border-4 border-primary bg-background rounded-2xl shadow-lg">
            <div class="flex justify-between items-center mb-4">
                ...
                <div class="mb-3">
                    <p type="text" name="Name" placeholder="Nama" value="{{ old('Name') }}"
                        class="w-full h-11 border-4 border-primary rounded-xl rounded-bl-3xl 
                            px-4 outline-none transition-all focus:border-secondary duration-300 ease-in-out
                            placeholder-gray-500 font-sans text-base">
                ...
                <div class="mb-4">
                    <p type="text" name="Phone_Number" placeholder="Nomor HP" value="{{ old('Phone_Number') }}"
                        class="w-full h-11 border-4 border-primary rounded-xl rounded-bl-3xl 
                            px-4 outline-none transition-all focus:border-secondary duration-300 ease-in-out
                            placeholder-gray-500 font-sans text-base" >
                ...
                <div class="mb-4">
                    <p type="text" name="Notes" placeholder="Catatan" value="{{ old('Notes') }}"
                        class="w-full h-11 border-4 border-primary rounded-xl rounded-bl-3xl 
                            px-4 outline-none transition-all focus:border-secondary duration-300 ease-in-out
                            placeholder-gray-500 font-sans text-base">
                ...
                @csrf
                <div class="flex flex-col items-center">
                <form action="{{ route('make.customer', $table->No_Table) }}" class="w-full" method="POST">
