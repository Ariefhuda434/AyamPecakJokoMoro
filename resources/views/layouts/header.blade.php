@php
    ...
    $dashboardRoute = match($roleSlug) {
        ...
        'waiter' => route('customer.index'), 
        'cashier' => route('cashier.view'),
        default => '/' 
    };
    
    $isActive = function ($routes) use ($dashboardRoute, $roleSlug) {
        if ($routes === $dashboardRoute || $routes === 'dashboard') {
            if ($roleSlug === 'waiter') {
                return Request::is('order*') || Request::is('order/*');
            }
            return Request::is(ltrim(parse_url($dashboardRoute, PHP_URL_PATH), '/')) || Request::is('dashboard');
        }
...
<div x-data="{ isSidebarOpen: true }" class="min-h-screen"> 
    ...
    <div :class="{'ml-64': isSidebarOpen, 'ml-0': !isSidebarOpen}"
        ...
        <div class="flex items-center gap-4">
            ...
            <button class="relative p-1 rounded-full hover:bg-gray-100 transition duration-150">
                ...
                <span class="absolute top-1 right-1.5 w-2 h-2 bg-secondary rounded-full border border-white"></span>
            </button>
            ...
            <div class="cursor-pointer">
                ...
                <span class="inline-flex items-center justify-center h-8 w-8 rounded-full bg-primary text-white text-xs ml-2">
                    {{ strtoupper(substr(Auth::user()->name_employee ?? 'P', 0, 1)) }}
                </span>
            </div>
    ...
    <main @click="if (isSidebarOpen && window.innerWidth < 768) isSidebarOpen = false"
          :class="{'ml-64': isSidebarOpen, 'ml-0': !isSidebarOpen}" 
          class="min-h-screen transition-all duration-300 ease-in-out pt-4 md:pt-6 p-4">
        @yield('content')
    </main>
